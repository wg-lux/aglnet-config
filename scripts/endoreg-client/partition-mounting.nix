{ config, lib, pkgs, network-config, partition-config, ... }:
...
let
    ...
    sopsfile-parent = ../../secrets +"/${hostname}";

    label = partition-config.label;
    conf = network-config.scripts.endoreg-sensitive-mounting."${label}";
    sopsfile = sopsfile-parent + "/${conf.keyfile-source}";

    partition-luks-uuid = partition-config.luks-uuid;
    sops-target = conf.sops-target;
...
{
  #   ####### KEYFILE #########
    # sops.secrets."dropoff-key" = {
    sops.secrets."${sops-target}" = {
        sopsFile = sopsfile;
        format = "binary";
        owner = user;
