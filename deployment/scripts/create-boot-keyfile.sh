...
NIX_CONFIG_PATH="./$HOSTNAME-usb-key.nix"
...
read -p "Please enter the device name (e.g., sdb) of the USB drive: " USB_DEVICE
USB_DEVICE_BY_MOUNT="/dev/$USB_DEVICE"
...
# sudo dd if=key.bin of= bs=1 seek=14900M count=4096

sudo dd if=$KEYFILE_NAME of=$USB_DEVICE bs=$BS count=$COUNT seek=$OFFSET_B

# Step 5: Prompt user for hostname configuration file, provide default value (HOSTNAME_CONFIG_PATH)
read -p "Please enter the path to the hostname configuration file (default: $HOSTNAME_CONFIG_PATH): " HOSTNAME_CONFIG_PATH

# Step 6: Add keyfile using cryptsetup
...
echo "NixOS configuration file created at $NIX_CONFIG_PATH"
